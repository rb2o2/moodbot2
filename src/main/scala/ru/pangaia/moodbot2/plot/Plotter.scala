...
class Plotter(config: Conf) {
  def plotPNGToFile(data: Seq[Mark]): String = {
    if data.isEmpty then throw new NoDataInPlotRangeException()
    ...
    p.getDomainAxis.setAutoTickUnitSelection(true)
    p.getDomainAxis.setTickMarksVisible(true)
    p.getRangeAxis.setRange(-2.5, 2.5)
    p.getRangeAxis.setTickMarksVisible(true)
    ...
    val tmp = Try(config("plot.filename.tmpdir")).getOrElse(System.getenv("TEMP"))
